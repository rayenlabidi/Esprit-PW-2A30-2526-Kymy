...
class FormationDataController
{
    ...
    public function all(array $filters = []): array
    {
        ...
        if (($filters['max_price'] ?? '') !== '' && ($filters['max_price'] ?? null) !== null) {
    ...
    {
        $sql = 'SELECT c.name AS category_name, COUNT(f.id) AS total_formations
                FROM categories c
                LEFT JOIN formations f ON f.category_id = c.id
                WHERE c.scope IN ("formation", "all")
                GROUP BY c.id, c.name
                ORDER BY total_formations DESC, c.name ASC';
